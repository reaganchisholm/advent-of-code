    ...
    for i in t:
        for j in expense_list:
            ...
            if val == 2020:
                expense_matched = (i * j)
                break
    return expense_matched

def part_two(array):
    expense_matched = None
    for i in expense_list:
        for j in expense_list:
            for h in expense_list:
                val = i + j + h
                if val == 2020:
                    expense_matched = (i * j * h)
                    break
    return expense_matched

answer_one = part_one(expense_list)
answer_two = part_two(expense_list)
